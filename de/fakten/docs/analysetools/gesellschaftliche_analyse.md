...
**Modellhinweis:** Die Singularität $\exists!$ gilt innerhalb einer Spezies. Sohn-Struktur ($\varepsilon_m \to 1$, $\Delta\varphi_m \to 0$) und Eva-Struktur ($\Delta\varphi_w = \delta\varphi_0 > 0$) sind nicht Folge von Leben, sondern seine resonanzlogische Bedingung (A4: ohne Phasendifferenz keine Schwingung, kein zeitlicher Prozess, kein Leben). Die Formel $\psi_{\text{Nachkomme}} = f(\psi_m, \psi_w, \Delta\varphi_{mw})$ gilt für jede Form von Leben – ohne Komplexitätsschwelle. 
