...
# Verwende einen interaktiven Matplotlib-Backend
matplotlib.use('TkAgg')  # Wähle den TkAgg-Backend, wenn du nicht in einer Notebook-Umgebung arbeitest
...
def animate(i: int, l1: float, l2: float, m1: float, m2: float, theta1_init: float, theta2_init: float) -> tuple[Any, Any]:
    ...
    # Berechne Positionen der Pendel für den aktuellen Zeitpunkt i
    x1, y1, x2, y2 = calculate_positions(l1, l2, m1, m2, theta1_init, theta2_init, 0.0, 0.0, t_span, t_eval[:i])
    
    # Lösche die vorherige Darstellung
    ax.clear()
    ax.set_xlim(-2, 2)
    ax.set_ylim(-2, 2)
...
def on_button_click(event: Any) -> None:
    try:
        ...
        # Animation starten und in einer Variablen speichern
...
# plt.show() anpassen, damit es nicht sofort ein Problem gibt
