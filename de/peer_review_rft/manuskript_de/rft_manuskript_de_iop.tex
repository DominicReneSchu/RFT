...
\begin{document}
	...
	\begin{abstract}
		...
		$\eta(\Delta\varphi) = \cos^2(\Delta\varphi/2)$ als emergente
		Eigenschaft aus der Klein-Gordon-Gleichung in expandierender
		Raumzeit folgt, mit linearer Skalierung
		$\mathrm{d}d_\eta/\mathrm{d}H_0 = 0{,}00204$~(km/s/Mpc)$^{-1}$
		\"uber 330~Simulationen und Verbesserung
		$\Delta\chi^2 = +16$ gegen\"uber Planck-2018-CMB-Daten;
	...
	%% ============================================================
	\subsection{Gekoppelte FLRW-Simulation: Kosmologische Validierung}
	\label{sec:flrw}
	%% ============================================================
	...
	Einfluss der Raumzeitexpansion.
	
	\subsubsection{Stufe 5: Falsifizierbare Vorhersage.}
	
	Ein Kontrolltest \"uber drei Szenarien zeigt, dass die
	Abweichung systematisch von der Expansionsrate abh\"angt:
	...
	\begin{center}
		\begin{tabular}{lrl}
			...
			Flach ($H = 0$) & $0{,}0438$
			& $\cos^2$ fast exakt \\
			FLRW ($\dot{a}_0 = 0{,}3$) & $0{,}1375$
			& $3\times$ gr\"o\ss{}er \\
			Schnell ($\dot{a}_0 = 1{,}0$) & $0{,}1812$
			& $4\times$ gr\"o\ss{}er \\
			\hline
		\end{tabular}
	\end{center}
	...
	\noindent
	\textbf{Best\"atigt:}
	$d_\eta(H{=}0) < d_\eta(H{>}0) < d_\eta(H{\gg}0)$.
	Die Hubble-Reibung ($3H\dot{\varepsilon}$-Term) reduziert
	$\eta$ systematisch unter $\cos^2(\Delta\varphi/2)$.
	...
	\begin{figure}[ht]
		...
		\caption{Phasenscan \"uber 20 initiale Phasendifferenzen
			$\Delta\varphi_0$: Simulationspunkte (blau) folgen der
			analytischen $\cos^2$-Kurve (rot) mit mittlerer
			Abweichung $d_\eta = 0{,}1394$.}
		\label{fig:phase_scan}
	\end{figure}
	
	\subsubsection{Stufe 6a: Kosmologische Skalierung.}
	
	Ein $H_0$-Scan \"uber 330~Einzelsimulationen
	($H_0 = 0$--$100$~km/s/Mpc, je 15~Phasenwerte) zeigt
	lineares Skalierungsverhalten:
	\begin{equation}
		d_\eta(H_0) = (0{,}00204 \pm 0{,}00003)\,
	...
	\begin{table}[ht]
		\centering
		\caption{Ergebnisse des $H_0$-Scans (330 Simulationen).
			Die Differenz zwischen Planck und SH0ES ist die erste
			quantitative Vorhersage der RFT f\"ur eine
			kosmologische Observable.}
		\label{tab:h0_scan}
		\begin{tabular}{lr}
			\hline
	...
	Die Differenz $\Delta d_\eta = 0{,}0114$ zwischen
	Planck~\cite{planck2018vi} und SH0ES~\cite{riess2022}
	stellt eine prinzipiell pr\"ufbare Vorhersage dar.
	
	\begin{figure}[ht]
		\centering
		\includegraphics[width=0.7\textwidth]%
		{figures/h0_scan.png}
		\caption{$H_0$-Scan \"uber 330~Simulationen. Links:
			$d_\eta$ als Funktion von~$H_0$ mit linearem Fit,
			Planck- und SH0ES-Markierungen. Rechts: Phasenscans
	...
	\begin{figure}[ht]
		...
		\caption{Resonanzfeld-Signatur der Hubble-Spannung:
			Die Differenz $\Delta d_\eta = 0{,}0114$ zwischen
			Planck ($H_0 = 67{,}4 \pm 0{,}5$) und SH0ES
			($H_0 = 73{,}0 \pm 1{,}0$) ist die erste quantitative
			Vorhersage der RFT f\"ur eine kosmologische Observable.}
	...
	\begin{itemize}
		...
		\item A3: $\eta = 1{,}0$ bei $\Delta\varphi = 0$
		\item A4: $\eta(\Delta\varphi) = \cos^2(\Delta\varphi/2)$
		emergiert
		\item A5: $a(t)$ reagiert auf Gesamtenergiedichte
		\item A7: Ergebnisse stabil \"uber 330~Simulationen
	\end{itemize}
	...
	\begin{center}
		\begin{tabular}{lll}
			...
			Kopplung & Keine Phasenabh\"angigkeit
			& $\eta(\Delta\varphi) = \cos^2(\Delta\varphi/2)$ \\
			$H_0$-Abh\"angigkeit & Parametersch\"atzung
			& $d_\eta$ skaliert linear mit $H_0$ \\
			Vorhersage & $H_0 = 67{,}4 \pm 0{,}5$
			& $\Delta d_\eta = 0{,}0114$ (Planck $\leftrightarrow$
			SH0ES) \\
			CMB-Fit & $\chi^2/\mathrm{dof} \approx 1$ (CAMB)
	...
	die $\eta$-Korrektur ($\Delta\chi^2 > 0$, Pearson~$r = 0{,}626$).
	...
	\begin{enumerate}
		...
		$\Delta d_\eta = 0{,}0114$ zwischen Planck und SH0ES
		muss in einen beobachtbaren Effekt auf das
		CMB-Leistungsspektrum oder die BAO-Skala
		\"ubersetzt werden.
	...
	\"uber $10\,000$~Durchl\"aufe (Axiom~3, 7).
	
	\textbf{Kosmologie:} Gekoppelte FLRW-Simulationen zeigen,
	dass die Kopplungseffizienz $\eta$ durch
	Hubble-Reibung systematisch unter
	$\cos^2(\Delta\varphi/2)$ gedr\"uckt wird.
	Die Skalierung $\mathrm{d}d_\eta/\mathrm{d}H_0 = 0{,}00204$
	\"uber 330~Simulationen und die Verbesserung
	$\Delta\chi^2 = +16$ gegen\"uber Planck-Daten
	stellen die erste kosmologische Vorhersage der RFT dar
	(Axiom~1, 3, 4, 5, 7).
	...
	$\ell$-Bereich und Boltzmann-Solver; und mehrj\"ahrige
