...
\begin{document}
	...
	\begin{abstract}
		...
		(4)~Schrödinger simulation (5~derivation steps) derives the
		Schrödinger equation from Axiom~4 with Fidelity~$= 1.0$
		(all~4 scenarios), perturbation theory confirmed
		$1-F \sim \lambda^2$, and delivers a falsifiable
		prediction for ${}^{87}$Rb.
		The application concept ResoTrade demonstrates the
		applicability of RFT axioms in a real market environment;
		primary axiom evidence comes from RFT-internal simulations.
		Two falsifiable experimental proposals are presented:
		(I)~phase-dependent photo-excitation of Am-241 at ELI-NP
		(nuclear physics, 0~free parameters, $>50{,}000\,\sigma$ legacy, corrected RT-09: SNR~$\approx 10\,\sigma$ at 100~h) and
	...
	reactor), and classical mechanics and quantum mechanics
	(double pendulum, Schrödinger simulation). Section~5
	...
	$\Delta d_\eta > 6\sigma$ (1\,530 runs).
	Schrödinger simulation: RFT-modified wave function confirmed
	with Fidelity~$= 1.0$ (correspondence principle, all~4 scenarios).
	...
	Coupled oscillators simulation: multi-frequency superposition
	confirmed (energy exchange at resonance).
	Schrödinger simulation: multi-mode superposition in coupled two-particle
	states confirmed; Gisin theorem (no-signaling) preserved.
	...
	\textit{Axiom status:} Corollary from the representation
	structure of $\mathbb{R}^+_\times \subset G_{\mathrm{sync}}$
	(RT-02, RT-35; see Section~\ref{sec:gsync}).
	A3 is not an independent axiom but a consequence of A7.
	
	\textit{Example:} Molecular orbital resonance,
	Tacoma Narrows bridge.
	Monte Carlo test: 5 resonances at particle masses,
	emp.\ $p = 0$ (1\,500\,000 simulations, 3 KDE, 30 seeds).
	In the resonance reactor: excitation frequency equals
	the GDR frequency of the nucleus.
	Schrödinger simulation: RFT energy levels confirm rational
	frequency ratios at resonance conditions.
	...
	\textit{Axiom status of the $\pi$ factor:} The factor~$\pi$
	is not a postulated free parameter, but the geometrically
	derived saddle-point contribution of the stationary action
	integral $S[\psi, \Delta\varphi]$ (RT-01, RT-01b;
	see Section~\ref{sec:pi_action}).
	The limit $E = \pi\varepsilon\hbar f \to E = hf_{\mathrm{Hz}}$
	is formally closed.%
	\footnote{Numerical confirmation (RT-01b): the path integral
	converges to $\pi$ to machine precision;
	non-Gaussian corrections
	$|c_3 + c_4| \approx 5.5\times10^{-11}$.}
	
	\textit{Axiom status of $\varepsilon = \cos^2(\Delta\varphi/2)$:}
	The function $\varepsilon(\Delta\varphi) = \cos^2(\Delta\varphi/2)$
	is representation-theoretically uniquely determined as the
	only function that belongs exclusively to the $k{=}1$
	representation of $\mathrm{U}(1) \subset G_{\mathrm{sync}}$
	(RT-02; see Section~\ref{sec:gsync}).
	The previous phenomenological formulation is thus replaced by
	this group-theoretic derivation.
	...
	Schrödinger simulation: correspondence principle with Fidelity~$= 1.0$;
	perturbation theory confirmed $1-F \sim \lambda^2$.
	Warp drive simulation: sign change in equation of state~$w$
	via $\varepsilon(\Delta\varphi)$ phase control.
	FLRW simulation: $\varepsilon = \eta = 0$ at $\Delta\varphi = \pi$
	(exact, 1\,530 runs).
	
	\subsection{Axiom 5: Energy Direction}
	
	\textit{Axiom status:} Irreducible structural postulate (RT-36).
	The $D$-generator of $G_{\mathrm{sync}}$ does not act on
	spatial directions: $\delta_D(\nabla\Phi/|\nabla\Phi|) \neq 0$
	in general. A5 is not derivable from $G_{\mathrm{sync}}$;
	the formal justification is provided by the vectoriality
	inconsistency of the scalar treatment of energy in torque,
	spin, and the Lorentz four-vector (RT-01a, RT-36).
	...
	\textit{Resonance field simulation:}
	Energy direction vector confirmed.
	Warp drive simulation: front/rear asymmetry
	(contraction vs.\ expansion) demonstrates directed energy flow.
	...
	\textit{Resonance field simulation:} Coupling efficiency
	and energy flow confirmed (PCI $\to$ MI).
	Schrödinger simulation: information propagates
	only along coherent phase paths — anti-resonant
	paths are systematically suppressed.
	
	\subsection{Axiom 7: Invariance under $G_{\mathrm{sync}}$}
	
	\textit{Axiom status:} Algebraically proved (stationary)
	for the frequency-ratio part and the phase-difference
	structure; domain transfer (CMB, nuclear physics,
	financial markets) remains a motivated postulate
	(RT-02; see Section~\ref{sec:gsync}).
	...
	\times \mathrm{Aff}^+(\mathbb{R})$ (algebraically proved,
	RT-02; see Section~\ref{sec:gsync}).
	
	\textit{Example:} Monte Carlo results stable over
	$50{,}000$ runs (3 KDE bandwidths, 30 seeds).
	FLRW simulation: resonance structure preserved across
	all 1\,530 parameter combinations ($H_0$, $\Omega$).
	
	\subsection{Axiom Status Overview (as of August 2026)}
	\label{sec:axiom_status}
	
	\begin{table}[ht]
		\centering
		\caption{Current axiom status of RFT (August 2026).}
		\label{tab:axiom_status}
		\begin{tabular}{lll}
			\hline
			\textbf{Axiom} & \textbf{Previous Status}
			& \textbf{Current Status} \\
			\hline
			A1 & Postulated & Postulated \\
			A2 & Postulated & Postulated \\
			A3 & Independent axiom
			& Corollary from A7 (RT-02, RT-35) \\
			A4: $\pi$ & Free parameter / postulate
			& Geometrically derived (RT-01, RT-01b) \\
			A4: $\varepsilon{=}\cos^2(\Delta\varphi/2)$ & Phenomenological choice
			& Representation-theoretically unique (RT-02) \\
			A5 & Postulate
			& Irreducible postulate (RT-36) \\
			A6 & Postulated & Postulated \\
			A7 & Postulated invariance
			& Algebraically proved, stationary (RT-02) \\
			\hline
		\end{tabular}
	\end{table}
	...
	\subsection{Action-Integral Derivation of $\pi$}
	\label{sec:pi_action}
	
	The factor $\pi$ in Eq.~(\ref{eq:grundformel}) is derived
	via an action functional of resonance coupling.
	
	\textbf{Action functional.}
	The coupling energy is formulated as an integral over the
	resonance coupling path in phase space:
	\begin{equation}
		S[\psi, \Delta\varphi]
	...
	\begin{equation}
		...
		\label{eq:lagrangian}
	\end{equation}
	The integration limits $[0,\pi]$ correspond to the
	physically complete transition from maximum resonance
	($\Delta\varphi = 0$, $\varepsilon = 1$) to destructive
	interference ($\Delta\varphi = \pi$, $\varepsilon = 0$).
	
	\textbf{Saddle-point approximation and $\pi$ contribution.}
	In the saddle-point approximation the path integral is
	dominated by the classical solution $\psi_0$. The
	classical action contribution is:
	\begin{equation}
		S[\psi_0]
	...
	$\delta\psi = \psi - \psi_0$ additionally yields a factor
	$\sqrt{\pi}$ from the circular structure of phase space.
	Both contributions share the same geometric origin:
	\begin{equation}
	...
	\textbf{Main result.}
	$\pi$ arises as the geometric saddle-point contribution
	of the phase-space integral over one half-period — not
	introduced as a free parameter, but obtained as the result
	of integrating the coupling geometry.
	
	\textbf{Corollary: Limit to the Planck relation.}
	For $\varepsilon = 1/(2\pi)$ (Planck ground state)
	and $\omega = 2\pi f_{\mathrm{Hz}}$ it follows that:
	\begin{equation}
	...
	\end{equation}
	The standard Planck relation $E = hf_{\mathrm{Hz}}$
	corresponds to the state $\varepsilon = 1/\pi$.
	
	\textbf{Falsification condition.}
	If $S[\psi, \Delta\varphi]$ does not yield $\pi$ as a
	saddle-point contribution, Axiom~A4 must be reformulated.
	...
	\subsection{Group-Theoretic Proof of $G_{\mathrm{sync}}$}
	\label{sec:gsync}
	
	\subsubsection{Group structure.}
	The symmetry group of RFT is (RT-02):
	\begin{equation}
		G_{\mathrm{sync}} \cong
		\mathbb{R}^+_\times \times \mathrm{U}(1)
		\times \mathrm{Aff}^+(\mathbb{R})
		\label{eq:gsync_structure_en}
	\end{equation}
	The three factors arise from:
	\begin{itemize}
		\item $\mathbb{R}^+_\times$: frequency scaling
		(scale invariance of resonance, $f \mapsto \alpha f$,
		$\alpha > 0$); generates the quantisation corollary A3.
		\item $\mathrm{U}(1)$: phase rotations
		($\Delta\varphi \mapsto \Delta\varphi + \phi_0$);
		determines $\varepsilon$ uniquely.
		\item $\mathrm{Aff}^+(\mathbb{R})$: time reparametrisation
		($t \mapsto at + b$, $a > 0$); ensures observer independence.
	\end{itemize}
	The full algebraic proof (Lie algebra,
	group homomorphisms, stationarity conditions)
	is given in the source document~\cite{rftrepo} (RT-02).
	
	\subsubsection{Uniqueness of $\varepsilon$.}
	On $\mathrm{U}(1)$ the irreducible representations are
	$\rho_k\colon e^{i\varphi} \mapsto e^{ik\varphi}$, $k\in\mathbb{Z}$.
	The only real $2\pi$-periodic function satisfying:
	(i)~symmetry $\varepsilon(\Delta\varphi) = \varepsilon(-\Delta\varphi)$,
	(ii)~normalisation $\varepsilon(0)=1$, $\varepsilon(\pi)=0$,
	(iii)~membership in the $k{=}1$ representation, is:
	\begin{equation}
		\varepsilon(\Delta\varphi)
		= \cos^2\!\!\left(\frac{\Delta\varphi}{2}\right)
		\label{eq:eps_unique_en}
	\end{equation}
	This replaces the previous phenomenological postulate
	of this functional form.
	
	\subsubsection{A3 as corollary.}
	The subgroup $\mathbb{R}^+_\times$ acts on frequency pairs
	$(f_1, f_2)$ by simultaneous rescaling.
	Fixed points of the stabiliser of any resonance state
	are exactly the rational frequency ratios~$n/m$.
	Hence A3 (resonance condition) is not an independent axiom
	but a corollary of the $\mathbb{R}^+_\times$ factor of A7
	(Korollar aus A7, RT-02, RT-35).
	
	\subsubsection{Lie algebra and invariance structure.}
	\begin{center}
		\begin{tabular}{lll}
			\hline
			\textbf{Generator} & \textbf{Action}
			& \textbf{Axiom consequence} \\
			\hline
			$D$ (dilation) & $f\mapsto\alpha f$ & A3 corollary \\
			$R$ (rotation) & $\Delta\varphi\mapsto\Delta\varphi+\phi_0$
			& A4: $\varepsilon$ unique \\
			$T, S$ (affine) & $t\mapsto at+b$
			& A7: observer independence \\
			\hline
		\end{tabular}
	\end{center}
	...
	The resonance masses agree with PDG~2022 values~\cite{pdg2022}
	and the CMS dielectron spectrum~\cite{cms_dielectron}.
	...
	\begin{equation}
		...
		\label{eq:kg_flrw}
	\end{equation}
	The extended nonlinear potential is (RT-32):
	\begin{equation}
	...
	\end{equation}
	The parameter $\lambda_{\varepsilon^4}$ (implemented as
	\texttt{lambda\_eps4}) is the sixth-order saturation term.
	Perturbation theory yields:
	$\delta\eta \approx -c \cdot \lambda_{\varepsilon^4}
	\cdot \langle\varepsilon^3\rangle$.
	For $\lambda_{\varepsilon^4} = 0$ the potential reduces
	to the standard $\lambda\varphi^4$ potential
	...
	\begin{equation}
		...
		\label{eq:friedmann}
	\end{equation}
	\textit{Note on $\kappa$:} The RFT simulations use the
	modified Friedmann equation with $\kappa_{\mathrm{RFT}} = 1$.
	This value is not a derivation from axioms~A1--A7, but a
	normalisation convention for the dimensionless simulation
	unit system (RT-11). For quantitative comparison with
	cosmological observables, $\kappa$ must be replaced by
	the physical value $8\pi G/c^4$; the qualitative results
	($\eta$ correction, $d_\eta$ scaling) are $\kappa$-invariant.
	...
	system built exclusively on the axioms of RFT and
	demonstrates their applicability in a real market
	environment. Since ResoTrade is a private
	implementation, it does not serve as primary axiom
	evidence; primary evidence comes from the RFT-internal
	simulations (FLRW, Monte Carlo, double pendulum,
	resonance reactor).
	
	\subsubsection{RFT-internal Axiom Validation.}
	...
	\begin{center}
		...
		\begin{tabular}{lll}
			...
			A2 & Coupled oscillators
			& Multi-frequency superposition \\
			A3 & Monte Carlo (1\,500\,000 sim.)
			& 5 resonances, emp.\ $p = 0$ \\
			A4 & FLRW: $\varepsilon = \eta$
			& $\kappa = 1$, no free param. \\
			A5 & Resonance field sim.
			& Energy direction vector \\
			A6 & Resonance field sim.
			& Coupling efficiency, energy flow \\
			A7 & Monte Carlo
			& Bandwidth-independent (3 KDE) \\
			\hline
		\end{tabular}
	\end{center}
	...
	\subsubsection{Application Concept: Backtest and Live Results.}

	\emph{Note: The 24-month backtest and 5-day live validation
	results below are not available in the repository (private
	implementation). This section describes ResoTrade as an
	application concept of RFT applied to financial markets,
	not as peer-review-ready empirical evidence. Full
	reproducibility (code, raw data, trade logs) is an open task.}
	...
	point, the theoretical significance under ideal phase coherence is
	$> 50{,}000\,\sigma$. Realistic corrections are required for:
	detector efficiency, beam divergence, nuclear state width
	$\Gamma_{\mathrm{GDR}} \approx 4$--5~MeV, and experimentally
	achievable phase coherence. A complete uncertainty budget is
	outstanding (see supplementary open research tasks).
	Total beam time: ${\sim}30$~h, cost: ${\sim}50{,}000$~EUR.
	...
	\begin{table}[ht]
		...
		\begin{tabular}{ll}
			...
			Significance & $> 50{,}000\,\sigma$ (theoretical,
			ideal phase coherence; full uncertainty budget
			outstanding) \\
			Free parameters & $0$ \\
			\hline
		\end{tabular}
	...
	\emph{Note on falsifiability: $\lambda$ is a free parameter
	of RFT without an independently determined value. The
	prediction is parametric: any measurement result $>0$
	would be compatible with a suitable $\lambda$. The prediction
	becomes falsifiable only through independent determination of
	$\lambda$ from a second, separate experiment.}
	...
	\begin{enumerate}
		\item \textbf{Determination of $\varepsilon$ in further domains:}
		Ab-initio derivation from system geometry in non-harmonic systems.
		...
		\item \textbf{Independent $\eta$ estimators (RT-07):}
		Energy transfer rate, mutual information, and phase locking value
		as alternatives to the Pearson estimator — independent of
		$\cos^2(\Delta\varphi/2)$ — to strengthen falsifiability.
		
		\item \textbf{$\hat{H}_{\mathrm{res}}$:}
		Construct the resonance Hamiltonian~(\ref{eq:h_res})
		for specific systems.
		
		\item \textbf{$\kappa$ in SI units (RT-04):}
		FLRW solver with $\kappa = 8\pi G/c^4$ and Planck-2018 parameters.
	...
	\textbf{Classical mechanics / quantum mechanics:}
	Double pendulum simulation confirms
	$\varepsilon(\theta_2-\theta_1) = \cos^2(\Delta\theta/2)$
	dynamically. Schrödinger simulation derives the
	Schrödinger equation from Axiom~4 with Fidelity~$= 1.0$
	(all~4 scenarios), perturbation theory confirmed
	$1-F \sim \lambda^2$, and delivers a falsifiable
	prediction for ${}^{87}$Rb (Axiom~1--7).
	Warp drive simulation: first positive-energy warp bubble
	($E^- = 0$), sign change in equation of state~$w$
	via phase control (Axiom~4, 5).
	...
	\begin{thebibliography}{99}
		...
		\textit{Prog.\ Theor.\ Exp.\ Phys.}\
		\textbf{2022} 083C01
		
		\bibitem{cms_dielectron}
		CMS Collaboration 2019
		\textit{Search for new physics in dielectron final states},
		CMS Physics Analysis Summary CMS-PAS-EXO-19-019,
		\url{https://cds.cern.ch/record/2684757}
