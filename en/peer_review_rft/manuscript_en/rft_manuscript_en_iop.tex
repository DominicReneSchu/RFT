...
\begin{document}
	...
	\begin{abstract}
		...
		(4)~ResoTrade demonstrates as an application concept
		the applicability of RFT axioms in a real market
		environment ($+26.3\%$ vs.\ HODL over 24~months
		across 4~market regimes); primary axiom evidence
		comes from RFT-internal simulations (FLRW, Monte Carlo,
		resonance reactor, double pendulum).
	...
	reactor), and classical mechanics and application
	concepts (double pendulum, ResoTrade). Section~5
	...
	Application example (ResoTrade): AC/DC decomposition of the BTC price field
	...
	Application example (ResoTrade): Three superposed modes
	...
	Application example (ResoTrade): $\mathrm{MA}_{\mathrm{SHORT}}$ (24h)
	...
	\textit{Application example (ResoTrade):} Downtrend pause gate --
	...
	\textit{Application example (ResoTrade):}
	...
	\textit{Application example (ResoTrade):} Resonance gate -- trades only
	...
	Application example (ResoTrade): positive in
	...
	\textbf{Classical mechanics / application concepts:}
	Double pendulum simulation confirms
	$\varepsilon(\theta_2-\theta_1) = \cos^2(\Delta\theta/2)$
	dynamically. ResoTrade demonstrates as an application
	concept the applicability of RFT axioms in a real
	BTC market environment ($+26.3\%$ vs.\ HODL across
	4/4~market regimes). Primary axiom evidence comes
	from RFT-internal simulations (Axiom~1--7).
