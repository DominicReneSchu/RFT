...
root = tk.Tk()
root.title("Double Pendulum Simulation with Sliders")
...
m1_slider = tk.Scale(root, from_=0.1, to=5, orient="horizontal", length=300, label="Mass 1 (m1)", command=lambda e: update_plot())
m1_slider.set(1.0)

m2_slider = tk.Scale(root, from_=0.1, to=5, orient="horizontal", length=300, label="Mass 2 (m2)", command=lambda e: update_plot())
m2_slider.set(1.0)

l1_slider = tk.Scale(root, from_=0.1, to=2, orient="horizontal", length=300, label="Length 1 (l1)", command=lambda e: update_plot())
l1_slider.set(1.0)

l2_slider = tk.Scale(root, from_=0.1, to=2, orient="horizontal", length=300, label="Length 2 (l2)", command=lambda e: update_plot())
l2_slider.set(1.0)

θ1_slider = tk.Scale(root, from_=-np.pi, to=np.pi, orient="horizontal", length=300, label="Angle 1 (θ1)", command=lambda e: update_plot())
θ1_slider.set(np.pi / 2)

θ2_slider = tk.Scale(root, from_=-np.pi, to=np.pi, orient="horizontal", length=300, label="Angle 2 (θ2)", command=lambda e: update_plot())
θ2_slider.set(np.pi / 2)

# Placement of sliders
