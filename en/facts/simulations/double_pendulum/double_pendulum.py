...
def coupling_efficiency(delta_phi: float | np.ndarray) -> float | np.ndarray:
    ...
    Maximum coupling at phase equality (Δφ = 0),
    no coupling at antiphase (Δφ = π).
    """
    return np.cos(delta_phi / 2) ** 2


class DoublePendulumSim:
    """Double pendulum with dynamic coupling efficiency.

    The Lagrange equations contain the natural mechanical
    coupling. Additionally, the coupling efficiency
    ε(θ₂−θ₁) = cos²((θ₂−θ₁)/2) modulates a resonance coupling term
    with amplitude A (slider parameter).
    ...
    def __init__(self) -> None:
        ...
        self.current_epsilon = 1.0  # updated per step
        self.reset()
    ...
    def derivatives(self, t: float, state: np.ndarray, m1: float, m2: float, L1: float, L2: float, amplitude: float) -> list[float]:
        ...
        # Coupling efficiency (Axiom 4)
        eps = coupling_efficiency(delta)

        denom1 = (m1 + m2) * L1 - m2 * L1 * np.cos(delta)**2

        # Resonance coupling term: A · ε(Δφ) · sin(Δφ)
    ...
    def reset(self) -> None:
        ...
        self.trail2_x = []
        self.trail2_y = []
        self.current_epsilon = coupling_efficiency(
    ...
    def step(self, dt_step: float, amplitude: float) -> None:
        ...
        # Update coupling efficiency
        delta = self.state[2] - self.state[0]
        self.current_epsilon = coupling_efficiency(delta)
    ...
    def get_energies(self, amplitude: float) -> tuple[float, float, float]:
        """Kinetic, potential, and coupling energy."""
        ...
        # Coupling energy scaled with ε and amplitude
    ...
    def get_kappa(self, amplitude: float) -> float:
        """Coupling ratio κ = E_coupling / |E_total|."""
...
fig.canvas.manager.set_window_title(
    'Double Pendulum — Resonance Field Theory (A1, A2, A4)')
...
fig.text(0.5, 0.98, "Double Pendulum — Resonance Field Theory",
         ha='center', va='top', fontsize=15,
         color="#224488", weight='bold')
info_text_header = (
    r"$\theta_1$, $\theta_2$: Angles | "
    r"$m_1$, $m_2$: Masses | "
    r"$L_1$, $L_2$: Lengths | "
    r"$A$: Coupling amplitude | "
    r"$\varepsilon(\Delta\varphi) = \cos^2(\Delta\varphi/2)$: "
    r"Coupling efficiency (dynamic)"
)
...
slider_labels = [
    ...
    ('Trail length', 50, 1000, 'trail_length'),
]
...
gif_button = Button(gif_button_ax, 'Export GIF')
...
def export_gif(event: Any) -> None:
    ...
    print("GIF exported: doppelpendel.gif")
