"""
3D resonance field simulation — GPU variant (CuPy).

Identical physics to field_3d.py, but using CuPy arrays
for GPU-accelerated computation.

Requirement: pip install cupy (+ CUDA-capable GPU)

Dependencies: cupy
"""

try:
    import cupy as cp
except ImportError:
    cp = None


def field_3d_sim_gpu(
    N=64, L=10.0, dt=0.004, steps=300, m=1.0, lmbda=0.2,
    initial_bump_size=5, bump_value=1.0, callback=None
):
    """3D simulation on the GPU via CuPy.

    Parameters identical to field_3d.field_3d_sim.
    Callback receives numpy arrays (automatically copied from GPU).

    Raises
    ------
    ImportError
        If CuPy is not installed.
    """
    if cp is None:
        raise ImportError(
            "CuPy is not installed. "
            "Please run 'pip install cupy' (CUDA GPU required)."
        )

    dx = L / N
    eps = cp.zeros((N, N, N), dtype=cp.float32)
    ix = iy = iz = N // 2
    half = initial_bump_size // 2
    eps[ix-half:ix+half+1, iy-half:iy+half+1, iz-half:iz+half+1] = bump_value
    eps_old = eps.copy()

    for step in range(steps):
        lap = (
            cp.roll(eps, 1, axis=0) + cp.roll(eps, -1, axis=0) +
            cp.roll(eps, 1, axis=1) + cp.roll(eps, -1, axis=1) +
            cp.roll(eps, 1, axis=2) + cp.roll(eps, -1, axis=2) -
            6 * eps
        ) / dx**2
        Vp = m**2 * eps + lmbda * eps**3
        eps_new = 2 * eps - eps_old + dt**2 * (lap - Vp)

        # Dirichlet boundary conditions
        eps_new[0, :, :] = 0
        eps_new[-1, :, :] = 0
        eps_new[:, 0, :] = 0
        eps_new[:, -1, :] = 0
        eps_new[:, :, 0] = 0
        eps_new[:, :, -1] = 0

        eps_old, eps = eps, eps_new

        if callback is not None:
            callback(cp.asnumpy(eps), step)

    return cp.asnumpy(eps)
