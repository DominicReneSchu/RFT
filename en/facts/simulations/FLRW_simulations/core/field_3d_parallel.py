"""
3D resonance field simulation — Numba-parallelised variant.

Identical physics to field_3d.py, but using @njit(parallel=True)
for significantly faster computation on multi-core CPUs.

Requirement: pip install numba
...
def step_field_3d_parallel(eps, eps_old, m, lmbda, dt, dx):
    """One time step of the 3D wave equation (parallelised).

    Parameters
    ----------
    eps, eps_old : ndarray, shape (N, N, N)
        Current and previous field configuration
    m, lmbda : float
...
def field_3d_sim_parallel(
    ...
    """3D simulation with Numba parallelisation.

    Parameters and return value identical to field_3d.field_3d_sim.
    """
    dx = L / N
    eps = np.zeros((N, N, N))
    ix = iy = iz = N // 2
