"""Visualisation Level 6a: d_eta(H0) — prediction curve."""
...
def plot_h0_scan(fit_result, save_path=None, show=True):
    """Plots d_eta as a function of H0 with linear fit.

    Two panels:
        Left:  d_eta(H0) with fit and Planck/SH0ES markers
        Right: Individual eta phase scans for selected H0 values
    """
    ...
    ax.axhline(
        ...
        label=f"Flat (H=0): d_eta = {d_flat:.4f}",
    )

    # Planck and SH0ES markers
    ...
    ax.set_ylabel("<|d_eta|>  (mean deviation from cos^2)", fontsize=12)
    ax.set_title(
        "Resonance field theory: d_eta(H0)\n"
        "Prediction: Hubble friction shifts eta monotonically",
        fontsize=12, fontweight="bold",
    )
    ax.legend(fontsize=9, loc="upper left")
    ax.grid(True, alpha=0.3)

    # === Panel 2: Selected phase scans ===
    ax2 = axes[1]
    ...
    # Select 5 evenly distributed H0 values
    n_show = min(5, len(h0))
    ...
    ax2.set_ylabel("Coupling efficiency eta", fontsize=12)
    ax2.set_title(
        "Phase scans at various H0\n"
        "Stronger expansion -> stronger deviation",
        fontsize=12, fontweight="bold",
    )
    ...
    plt.suptitle(
        "Level 6a: Cosmological scaling of resonance field coupling",
...
def plot_hubble_tension(tension, fit_result, save_path=None, show=True):
    """Detail plot: Hubble tension signature."""
    ...
    ax.text(
        ...
        f"({tension['relative_shift']:.1f}% shift)",
    ...
    ax.set_title(
        "Resonance field signature of the Hubble tension\n"
