"""Comparison plot: flat spacetime vs. FLRW expansion."""
...
def plot_control_comparison(scan_flat, scan_flrw, scan_fast=None, save_path=None, show=True):
    ...
    ax.plot(dphi_fine, np.cos(dphi_fine / 2)**2, color="black", linewidth=2, linestyle="--", label="Theory: cos^2(dphi/2)", zorder=10)

    valid_flat = np.isfinite(scan_flat["eta_mean"])
    ax.scatter(dphi[valid_flat], scan_flat["eta_mean"][valid_flat], color="tab:blue", s=70, zorder=5, edgecolors="black", linewidths=0.5, marker="o", label="Flat (H = 0)")
    ...
    ax.set_title("Control test: eta(dphi) in different spacetimes", fontsize=12)
    ...
    if np.any(valid_flat):
        ...
        ax2.scatter(dphi[valid_flat], d_flat, color="tab:blue", s=70, edgecolors="black", linewidths=0.5, marker="o", label=f"Flat: <|d_eta|> = {np.mean(np.abs(d_flat)):.4f}")
    ...
    if scan_fast is not None and np.any(valid_fast):
        ...
        ax2.scatter(dphi[valid_fast], d_fast, color="tab:orange", s=70, edgecolors="black", linewidths=0.5, marker="D", label=f"Fast: <|d_eta|> = {np.mean(np.abs(d_fast)):.4f}")

    ax2.axhline(0, color="black", linewidth=1, linestyle="--")
    ax2.set_xlabel("Phase difference dphi_0  [rad]", fontsize=12)
    ax2.set_ylabel("d_eta = eta_sim - cos^2(dphi/2)", fontsize=12)
    ax2.set_title("Spacetime signature: deviation d_eta\nPrediction: d_eta(H=0) ~ 0, d_eta(H>0) > 0", fontsize=12)
    ax2.legend(fontsize=10); ax2.grid(True, alpha=0.3)
    ax2.set_xlim(-0.1, np.pi + 0.1)

    plt.suptitle("Falsifiable prediction of resonance field theory:\nHubble friction shifts eta systematically above cos^2(dphi/2)", fontsize=13, fontweight="bold")
    plt.tight_layout()
