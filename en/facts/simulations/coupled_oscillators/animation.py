"""
Animation and visualisation of the coupled oscillators.
...
def smooth_max(arr: float | np.ndarray, window: int) -> float | np.ndarray:
    """Maximum of the last `window` values. Also works for scalars."""
    arr = np.asarray(arr)
    if arr.ndim == 0:
        return arr
    if len(arr) < window:
...
def init(
    ...
    """Reset all plot elements to initial state."""
    for line in [
...
def update(
    ...
    ax_traj.set_title("Oscillator Trajectories")
    ax_sin.set_title("Displacement over Time")
