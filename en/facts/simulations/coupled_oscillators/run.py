"""
Coupled Oscillators — Interactive Simulation (Axioms A1–A4)

Entry point with UI, sliders, and animation.
...
def main() -> None:
    ...
    fig.canvas.manager.set_window_title(
        'Coupled Oscillators — Resonance Field Theory (A1–A4)')
    ...
    f1_slider = Slider(ax_f1, 'Frequency 1 (Hz)', 0.7, 1.3,
                       valinit=1.0, valstep=0.005)
    f2_slider = Slider(ax_f2, 'Frequency 2 (Hz)', 0.7, 1.3,
                       valinit=1.02, valstep=0.005)
    alpha_slider = Slider(ax_alpha, 'α (Coupling Sharpness)', 0.1, 8.0,
                          valinit=alpha_0, valstep=0.01)
    tol_slider = Slider(ax_tol, 'Tolerance', 0.01, 0.5,
                        valinit=0.1, valstep=0.01)
    speed_slider = Slider(ax_speed, 'Speed', 10, 200,
    ...
    # --- Sine plot ---
    sinus_line1, = ax_sin.plot([], [], 'b-', label='Oscillator 1')
    sinus_line2, = ax_sin.plot([], [], 'r-', label='Oscillator 2')
    ...
    ax_sin.set_title("Displacements")

    # --- Energy plot ---
    kin_line, = energy_ax.plot([], [], color='blue', label='kinetic')
    ...
    energy_ax.set_title("Energy Evolution (A4)")

    # --- Coupling operator plot ---
    eps_line, = coupling_ax.plot([], [], color='red',
                                 label='ε — Coupling Operator')
    ...
    coupling_ax.set_xlabel("Time [s]")
    coupling_ax.set_ylabel("ε / E_res")
    coupling_ax.legend(fontsize='small')
    coupling_ax.set_title("Coupling Operator ε and Resonance Energies")

    # --- Resonance divergence plot ---
    resdiv_line, = resdiv_ax.plot([], [], color='green',
                                   label='Resonance Divergence')
    resdiv_ax.set_xlim(t[0], t[-1])
    resdiv_ax.set_ylim(0, 1.1)
    resdiv_ax.set_xlabel("Time [s]")
    resdiv_ax.set_ylabel("ΔE")
    resdiv_ax.legend(fontsize='small')
    resdiv_ax.set_title("Resonance Divergence |E_mech − E_res|")
    ...
    def recalc_interpolators() -> None:
        ...
        if is_res:
            fig.suptitle(
                ...
                f'✓ Resonance: f₁/f₂ ≈ {n}/{m_r}',
                fontsize=12, color='green')
        else:
            fig.suptitle(
                f'Coupled Oscillators — '
                f'✗ No Resonance: f₁/f₂ ≈ {f1/f2:.3f}',
    ...
    def on_export_gif(event: Any) -> None:
        ...
        btn_gif.label.set_text("GIF exported!")
