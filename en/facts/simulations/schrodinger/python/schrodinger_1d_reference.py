"""
1D Schrödinger Reference Simulation (Free particle + optional potential)
Purpose: Reference model for later resonance/phase lattice models.

Numerics: Split-operator (FFT), unitary -> good norm conservation.
Dependencies: numpy, matplotlib (optional for plot).

Units:
- Dimensionless by default with ħ = 1, m = 1.
- Then E(k) = k^2 / (2m) = k^2/2 and time is scaled accordingly.
...
def expectation_p_from_x(psi_x: np.ndarray, k: np.ndarray, dx: float, hbar: float) -> float:
    """
    Second independent check of <p> directly in position space via derivative operator.

    Computes <p> = Re( ∫ ψ*(x) (-iħ ∂_x) ψ(x) dx ).

    Implementation: derivative via FFT (spectrally exact for periodic boundary conditions):
        ∂_x ψ = IFFT( i k · FFT(ψ) )

    Note: The x-grid has periodic boundary conditions (implied by FFT).
    For a well-localized wavepacket that does not reach the boundary, this
    result agrees with the k-space expectation value.
    """
...
def expectation_energy_from_k_and_x(
    ...
    """
    Computes the energy expectation value ⟨H⟩ = ⟨T⟩ + ⟨V⟩.

    ⟨T⟩ is computed in k-space:    ⟨T⟩ = ∫ (ħk)²/(2m) |ψ(k)|² dk
    ⟨V⟩ is computed in position space: ⟨V⟩ = ∫ V(x) |ψ(x)|² dx
    """
    T_k = (hbar * k) ** 2 / (2.0 * m)
...
def main() -> int:
    ...
    print(f"<p>(t0)={p_mean0:.6f}  <p>(t_end)={p_means[-1]:.6f}  [k-space]")
    print(f"<p>(t0)={p_mean0_x:.6f}  <p>(t_end)={p_means_x[-1]:.6f}  [x-space via derivative]")
    ...
    if args.V == "free":
        ...
        if abs(p_means[-1] - target_p) > 0.5:
            print(f"[smoke] unexpected <p> (k-space): got {p_means[-1]:.6f}, expected ~ {target_p:.6f}")
            return 3
        if abs(p_means_x[-1] - target_p) > 0.5:
            print(f"[smoke] unexpected <p> (x-space): got {p_means_x[-1]:.6f}, expected ~ {target_p:.6f}")
            return 3
