...
# Expected hit rates (example values; adjust or determine dynamically if necessary)
expected_hit_rates = {
    1: 0.01,
    0.5: 0.005,
    2/3: 0.006,
...
print(f"Estimated background rate outside the signal regions: {background_rate:.5f}\n")
...
for eps in epsilons:
    ...
    for delta in deltas:
        ...
        # You can use background_rate instead of expected_hit_rates[eps] if there is no theoretical expectation
    ...
    print(f"Epsilon {eps}: Best Delta = {deltas[best_idx]:.3f} | Hits = {hits_list[best_idx]} out of {n} | "
          f"raw p-value = {p_values[best_idx]:.3e} | corrected p-value = {pvals_corrected[best_idx]:.3e}\n")

    # Plot of the uncorrected p-values
