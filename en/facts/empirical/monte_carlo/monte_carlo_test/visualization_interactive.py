...
def interactive_hits_histogram(sim_hits: dict[float, list[int]], real_hits: dict[float, dict[str, Any]], m0_values: list[float]) -> list[go.Figure]:
    """Interactive histograms: MC hits vs. real hits per M₀."""
    figs = []
    for m0 in m0_values:
        fig = go.Figure()
        fig.add_trace(
        ...
        fig.update_layout(
            title=f"M₀ = {m0:.2f} GeV: Monte Carlo hits vs. real hits",
...
def interactive_pval_curve(
    ...
    """Interactive p-value curves over Δ per M₀."""
    figs = []
    for m0 in m0_values:
        ...
        fig.update_layout(
            yaxis_type="log",
            title=f"p-value curve for M₀={m0:.2f} GeV",
            xaxis_title="Δ (GeV)",
...
def interactive_heatmap_contrast(
    ...
    """Interactive heatmap with Cividis colour scale."""
