...
def main() -> None:
    ...
    n = len(mass_data)
    print(f"Dataset loaded: {n} events (after NaN removal)")
    print(f"Mass range: {mass_data.min():.2f} – {mass_data.max():.2f} GeV")

    # --- Mask signal regions with narrow width ---
    ...
    print(f"Background data: {len(background_data)} events "
          f"({len(background_data)/n*100:.1f}%)")
    print(f"Excluded signal regions:")
    for m0 in M0_VALUES:
        excl = SIGNAL_EXCLUSION_WIDTH[m0]
        n_excl = np.sum((mass_data > m0 - excl) & (mass_data < m0 + excl))
        print(f"  M₀={m0:.2f} ± {excl:.2f} GeV: {n_excl} events removed")
    ...
    # --- Precompute expected rates ONCE ---
    print("\nPrecomputing expected rates (once) ...")
    expected_rates = precompute_expected_rates(kde_background, M0_VALUES, DELTAS)

    print(f"\nExpected rates from KDE background:")
