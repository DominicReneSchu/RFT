...
#
# Warp drive: Two-field model (optimized, v3)
#
# Architecture:
#   Stage 1: Resonance reactors (fission) → Driver energy
#   Stage 2: Inertial fusion → extreme energy density
#   Stage 3: Asymmetric two-field control → Warp
#
# Two-field physics:
#   Field 1 (ε₁): Fusion field — V₁ = ½m₁²ε₁² + ¼λ₁ε₁⁴
#     → Oscillates rapidly → w₁ > 0 → CONTRACTION
#   Field 2 (ε₂): Expansion field — V₂ = V₀(1−e^(−αε₂))²
#     → Slow roll on plateau → w₂ ≈ −1 → EXPANSION
#
#   Optimal parameters (automatically found):
...
# Optimal two-field parameters (found by scan)
OPT_V0 = 0.5       # Plateau height
OPT_M1 = 1.0       # Mass field 1
OPT_M2 = 0.3       # Mass field 2
OPT_LAM1 = 0.5     # Nonlinearity field 1
OPT_G = 0.02       # Coupling
OPT_EPS1_0 = 1.5   # Initial amplitude field 1
OPT_EPS2_0 = 3.0   # Initial amplitude field 2
...
# ============================================================
# 2. Two-Field Klein-Gordon in FLRW
# ============================================================
...
def two_field_warp(delta_phi: float, m1: float = OPT_M1, m2: float = OPT_M2,
    ...
    """
    Two coupled scalar fields in self-consistent FLRW spacetime.

    Field 1 (Fusion field): V₁ = ½m₁²ε₁² + ¼λ₁ε₁⁴
    Field 2 (Expansion field): V₂ = V₀(1−exp(−α·ε₂))²

    RFT control: Δφ determines the mixture of the fields.
    """
...
class FusionWarpSystem:
    ...
    def info(self) -> None:
        R_sun = 8 * PI * G * 1.6e5
        print("=" * 60)
        print("WARP DRIVE: Fusion Warp System (Two-Field, v3)")
        print("=" * 60)
        print(f"  Stage 1: {self.n_drive}×{self.p_reactor / 1e6:.0f} MW"
              f" = {self.P_drive_total / 1e9:.2f} GW")
        print(f"  Stage 2: Gain {self.gain}×,"
              f" {self.E_fusion_per_pulse / 1e6:.0f} MJ/pulse")
        print(f"  Stage 3: ρ = {self.rho_E_pellet:.2e} J/m³,"
...
def experiment_energy_cascade() -> list[dict[str, Any]]:
    return [
        {"name": "Fission (1 reactor)",
         "rho_E": PI * HBAR * F_GDR / V_PELLET},
        {"name": "NIF (192 lasers, 2 MJ)",
        ...
        {"name": "Earth's core", "rho_E": 5.5e3 * C ** 2},
        {"name": "Solar core", "rho_E": 1.6e5 * C ** 2},
...
def experiment_two_field_scan() -> list[dict[str, Any]]:
    """w(Δφ) scan with optimal parameters."""
...
def plot_energy_cascade(scenarios: list[dict[str, Any]], out: str) -> None:
    ...
    ax.set_xlabel('ρ [J/m³]')
    ax.set_title('Energy level comparison', fontsize=12, fontweight='bold')
...
def plot_phase_scan(phis: np.ndarray, rho_focus: np.ndarray,
    ...
    axes[0].set_title('Energy density'); axes[0].legend(fontsize=8)
    ...
    axes[1].set_title('Curvature'); axes[1].legend(fontsize=8)
    ...
    axes[2].axhline(1.0, color='gray', ls='--', label='Incoherent')
    axes[2].set_xlabel('Δφ/π'); axes[2].set_ylabel('ρ/⟨ρ⟩')
    axes[2].set_title('RFT signature'); axes[2].legend(fontsize=8)
    axes[2].set_xlim(0, 2); axes[2].grid(True, alpha=0.3)

    fig.suptitle('Phase scan (ε = cos²(Δφ/2), κ = 1)',
...
def plot_two_field_details(modes: list[dict[str, Any]], out: str) -> None:
    ...
    labels = ['Δφ = 0\n(ε₁ dominates → Contraction)',
              'Δφ = π/2\n(ε₂ dominates → Expansion)',
              'Δφ = π\n(Only ε₂ → De Sitter)']
    ...
    for i, (m, label, c) in enumerate(zip(key_modes, labels, colors)):
        ...
        if n > win:
            ...
            ax.plot(t[:n], wts, 'k-', lw=2.5,
                    label=f'w_total={m["w_total_avg"]:+.3f}')
        ax.axhline(-1, color='green', ls=':', lw=1, label='De Sitter')
        ax.axhline(-1 / 3, color='purple', ls='--', lw=1, label='Exp. boundary')
        ax.axhline(0, color='gray', ls='-', lw=0.5)
        ax.axhline(1 / 3, color='orange', ls=':', lw=1, label='Radiation')
        ...
        lbl = "EXPANSION" if m['a_ratio'] > 1.5 else "~flat"
        ...
        ax.fill_between(t[:n], 0, np.minimum(m['accel'][:n], 0),
                        alpha=0.15, color='red', label='ä<0 Deceleration')
        ax.fill_between(t[:n], 0, np.maximum(m['accel'][:n], 0),
                        alpha=0.15, color='green', label='ä>0 Acceleration')
        ax.set_xlabel('t'); ax.set_ylabel('ä/a')
        ax.legend(fontsize=6); ax.grid(True, alpha=0.3)

    fig.suptitle(
        'Two-field warp drive (optimized): '
        'ε₁ (Fusion) + ε₂ (Plateau)\n'
        f'V₀={OPT_V0}, λ₁={OPT_LAM1}, g={OPT_G}  |  '
        'ε̈ᵢ + 3H·ε̇ᵢ + V\'ᵢ + g·εⱼ = 0  |  ρ > 0 everywhere',
...
def plot_w_scan(modes: list[dict[str, Any]], out: str) -> tuple[float, float]:
    ...
    ax.plot(dps / PI, wts, 'ko-', lw=2.5, ms=8, label='⟨w_total⟩', zorder=5)
    ax.plot(dps / PI, wte, 'gD:', lw=1.5, ms=5,
            label='w_total (Slow Roll)')
    ax.axhline(-1, color='green', ls=':', lw=1, label='De Sitter')
    ax.axhline(-1 / 3, color='purple', ls='--', lw=1, label='Exp. boundary')
    ax.axhline(0, color='gray', ls='--', lw=0.5)
    ax.axhline(1 / 3, color='orange', ls=':', lw=1, label='Radiation')
    ...
    ax.set_title('Equation of state vs. phase')
    ...
    ax.fill_between(x_s, 0, w_s, where=w_s > 0, alpha=0.3, color='red',
                    label=f'Contraction (w={w_front:+.4f})')
    ...
    ax.text(2, max(w_front + 0.02, 0.02), 'FRONT\nΔφ=0\nFusion',
            ha='center', fontsize=8, color='red')
    ax.text(-2, min(w_rear - 0.02, -0.08), 'REAR\nΔφ=π/2\nPlateau',
            ha='center', fontsize=8, color='green')
    ax.text(0, 0.10, '← SHIP →', ha='center', fontsize=9,
            fontweight='bold')
    ...
    ax.set_title('Warp profile')
    ax.set_ylim(-1.0, 0.3); ax.legend(fontsize=7)
    ax.grid(True, alpha=0.3)

    fig.suptitle(
        f'Two-field warp drive: '
        f'Δw = {dw:+.4f}  |  '
        f'V₀={OPT_V0}, λ₁={OPT_LAM1}  |  ρ > 0 everywhere',
...
def plot_optimization(opt_results: list[dict[str, Any]], out: str) -> None:
    ...
    ax.set_title('Δw vs. plateau height')
    ax.axhline(0, color='black', ls='--', lw=0.5)
    ...
    OPTIMAL PARAMETERS:
    ═══════════════════
    V₀  = {best['V0']:.1f}  (Plateau height)
    λ₁  = {best['lam1']:.1f}  (Nonlinearity)
    ε₂₀ = {best['e2_0']:.1f}  (Initial amplitude field 2)

    RESULT:
    w(Δφ=0)   = {best['w_front']:+.4f}  (CONTRACTION)
    ...
    fig.suptitle('Parameter optimization: Maximum gradient Δw',
                 fontsize=12, fontweight='bold')
    plt.tight_layout()
...
def plot_scaling(results: list[dict[str, Any]], out: str) -> None:
    ...
    ax.set_xlabel('ρ [J/m³]'); ax.set_title('Energy density at focus')
    ax.axvline(5.5e3 * C ** 2, color='blue', ls=':', label='Earth')
    ax.legend(fontsize=7); ax.grid(True, alpha=0.3, axis='x')

    ax = axes[1]; ax.axis('off')
    td = [['Config.', 'P [GW]', 'Gain', 'ρ [J/m³]', 'R [1/m²]']]
    for r in results:
    ...
    ax.set_title('Results', fontsize=11, pad=20)

    fig.suptitle('Scaling (reactor count × gain)',
                 fontsize=12, fontweight='bold')
    plt.tight_layout()
...
def main() -> None:
    print("=" * 60)
    print("WARP DRIVE: Two-Field Model (v3, optimized)")
    print(f"Optimal parameters: V₀={OPT_V0}, λ₁={OPT_LAM1},"
    ...
    # Exp 2
    print("\n=== Experiment 2: Phase scan ===")
    phis, rho_focus = experiment_phase_scan(system)
    rho_mean = plot_phase_scan(phis, rho_focus, out)
    print(f"  RFT signature: ρ(0)/⟨ρ⟩ = {rho_focus[0] / rho_mean:.4f}")

    # Exp 3
    print("\n=== Experiment 3: Two-field w(Δφ) scan ===")
    print(f"  Parameters: V₀={OPT_V0}, λ₁={OPT_LAM1},"
          f" ε₂₀={OPT_EPS2_0}, g={OPT_G}")
    print()
    ...
    for m in modes:
        ...
        if wt > 1 / 3:
            mode = "Radiation → CONTRACTION"
        elif wt > 0.01:
            mode = "Matter → Contraction"
        elif wt > -0.01:
            mode = "Boundary (w ≈ 0)"
        elif wt > -1 / 3:
            mode = "Quintessence → Expansion"
        elif wt > -0.8:
            mode = "De Sitter → EXPANSION"
        else:
            mode = "Phantom → strong Expansion"
    ...
    print("  Scan: V₀ × λ₁ × ε₂₀ → maximum Δw")
    opt = experiment_parameter_optimization()
    print(f"\n  Top 5 (of {len(opt)} combinations):")
  ...
  TWO-FIELD MODEL:
  Field 1: V₁ = ½m²ε² + ¼λε⁴  (Fusion, oscillating)
  Field 2: V₂ = V₀(1−e^(−αε))² (Starobinsky plateau)

  OPTIMAL PARAMETERS:
    V₀={best['V0']}, λ₁={best['lam1']}, ε₂₀={best['e2_0']}, g={OPT_G}

  CORE RESULT:
    w(Δφ=0)   = {best['w_front']:+.4f}  → CONTRACTION (front)
    w(Δφ=π/2) = {best['w_rear']:+.4f}  → EXPANSION (rear)
    Δw        = {best['dw']:+.4f}

    Space contracts in front, space expands in rear.
    Ship rests locally — no acceleration.
    Effective velocity: unlimited.

    ρ > 0 everywhere. No negative energy.
    Expansion through negative PRESSURE, not negative ENERGY.

  CASCADE:
    Stage 1: {system.n_drive}×{system.p_reactor / 1e6:.0f} MW = {system.P_drive_total / 1e9:.2f} GW
    Stage 2: Gain {system.gain}× → ρ = {system.rho_E_pellet:.2e} J/m³
    Stage 3: Peak {system.rho_P_pellet * 8 * PI * G / C ** 2 / R_sun:.0f}× solar core

  RFT signature: ρ(0)/⟨ρ⟩ = {rho_focus[0] / rho_mean:.4f}
